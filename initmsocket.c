            ...
            for (int i = 0; i < MAX_SOCKETS; i++)
            {
                if (FD_ISSET(SM[i].sockfd, &readfds))
                {
                    ...
                    if (dropMessage(DROP_PROBABILITY))
                    {
                        if (type == 0)
                        {
                            printf("\033[0;34mDROPPED MSG: %d\n", seqnum);
                        }
                        else
                        {
                            printf("\033[0;34m\t\tDROPPED ACK: %d\n", seqnum);
                        }
                        break;
                    }
                    ...
                    if (SM[i].nospace)
                    {
                        printf("\033[0;34mNospace in REC-BUFFER, continuing...\n");
                        printf("REC-BUFFER: ");
                    ...
                    if (type == 0) // message type
                    {
                        ...
                        display[10] = '\0';
                        printf("\033[0;34mReceived MSG: %d\n", seqnum);
                        int pos = -1;
                        ...
                        if (pos != -1) // if message in rwnd
                        {
                            ...
                            printf("\033[0;31mACK %d sent\n", seqnum);
                            acksends++;
                        }
                        else
                        { // if message not in rwnd
                            printf("\033[0;34mMessage not in REC-WIND\n");
                            // send ACK
                            char ack[10];
                            sprintf(ack, "%d\n%d\n%d", 1, SM[i].rwnd.lastseqnum, SM[i].rwnd.window_size);
                            sendto(SM[i].sockfd, ack, sizeof(ack), 0, (const struct sockaddr *)&client_addr, client_len);
                    ...
                    {
                        SM[i].resends = 0;
                        printf("\033[0;34m\t\tReceived ACK: %d\n", seqnum);
                        int pos = -1;
...
void *sender(void *arg)
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < MAX_SOCKETS; i++)
        {
            ...
            if (time(NULL) - max_time > T && SM[i].resends < MAX_RESENDS)
            {
                for (int j = 0; j < SM[i].swnd.seqnumLen; j++)
                {
                    int pos = -1;
                    ...
                    int ret = sendto(SM[i].sockfd, (const char *)msg, sizeof(msg), 0, (const struct sockaddr *)&client_addr, sizeof(client_addr));
                    if (ret < 0)
                    {
                        perror("sendto");
                    }
            ...
            for (int j = 0; j < SEND_BUF_SIZE; j++)
            {
                ...
                if (present == 0)
                {
                    ...
                    printf("\033[0;31m\t\tSend MSG: %d\n", SM[i].sendBuf[j].seqnum);
                    SM[i].sendBuf[j].time = time(NULL);
                    msgsends++;
    ...
    while (1)
    {
        ...
        for (int i = 0; i < MAX_SOCKETS; i++)
        {
            ...
            if (SM[i].free == 0)
            {
                ...
                if (SM[i].resends < MAX_RESENDS && (!recBufEmpty || !sendBufEmpty))
                {
                    continue;
                }
